	...
	for( cf_target = ogl_target_start; cf_target <= ogl_target_end; ++cf_target )
	{
		if( buffer_index + DDS_full_size <= (unsigned int)buffer_length )
		{
			...
			if( uncompressed )
			{
				...
				for( i = 0; (unsigned)i < DDS_full_size; i += block_size )
				{
...
bool IsSupported( const std::string& extension )
{
    ...
    // Get a pointer to the glGetStringi method if it is supported.
...
int query_NPOT_capability( void )
{
	...
	if( has_NPOT_capability == SOIL_CAPABILITY_UNKNOWN )
	{
		...
		if( IsSupported( "GL_ARB_texture_non_power_of_two" ) )
		{
